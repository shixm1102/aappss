...
function _erasRewards(instanceId, api) {
  return (0, _util.memo)(instanceId, (accountId, eras, withActive) => {
    ...
    const cached = withActive
      ? []
      : eras.map((era) => _util.deriveCache.get(`${CACHE_KEY}-${era.toString()}`)).filter((value) => !!value);
    ...
    return api.query.staking.erasStakingPayout.multi(remainingAccountId).pipe(
      (0, _operators.map)((optRewards) => {
        const query = mapRewards(remaining, optRewards);
        !withActive && query.forEach((q) => _util.deriveCache.set(`${CACHE_KEY}-${q.era.toString()}`, q));

        return eras.map(
          (era) => cached.find((cached) => era.eq(cached.era)) || query.find((query) => era.eq(query.era))
        );
      })
    );
  });
...
function erasRewards(instanceId, api) {
  return (0, _util.memo)(instanceId, (accountId, withActive = false) =>
    api.derive.staking
      .erasHistoric(withActive)
      .pipe((0, _operators.switchMap)((eras) => api.derive.staking._erasRewards(accountId, eras, withActive)))
  );
}
