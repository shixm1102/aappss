...
Accounts with a balance lower than the minimal amount, 100 milliUnits (miliHEIMs for Rubik) as of writing are considered as nonexistent for the network. If an account's balance ever drops below this amount, it is removed from the network. In this application, it will still be visible, but with a balance of 0.
