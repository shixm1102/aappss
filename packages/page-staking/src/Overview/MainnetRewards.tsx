...
function MainnetReward({ children, className = "", label }: Props): React.ReactElement<Props> {
  ...
  const stakingRewards = new BN(37777.7777).mul(UNIT);
  const total = marketPayout && stakingRewards.add(new BN(Number(marketPayout).toString()));

  return (
    <div className={className}>
      {label || ""}
      <FormatBalance value={total} withSi label={<Icon icon="info-circle" tooltip={`mainnet-reward-trigger`} />}>
        <Tooltip
          text={
            <>
              <div>
                <div className="faded">
                  {" "}
                  {t("staking payout: {{stakingRewards}}", {
                    replace: {
